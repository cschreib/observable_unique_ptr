...
SUITE {
    ...
    TEMPLATE_LIST_TEST_CASE(
        "release valid owner with observer subobject", "[lifetime][release][owner][observer]",
        owner_types) {
        if constexpr (!is_sealed<TestType>) {
            memory_tracker mem_track;
    ...
    TEMPLATE_LIST_TEST_CASE(
        "object owning observer pointer to itself", "[lifetime][cycles][owner][observer]",
        owner_types) {
    ...
    TEMPLATE_LIST_TEST_CASE(
        "object owning observer pointer to other", "[lifetime][cycles][owner][observer]",
        owner_types) {
        if constexpr (is_cyclic<TestType>) {
            memory_tracker mem_track;
    ...
    TEMPLATE_LIST_TEST_CASE(
        "object owning observer pointer open chain", "[lifetime][cycles][owner][observer]",
        owner_types) {
        if constexpr (is_cyclic<TestType>) {
            memory_tracker mem_track;
    ...
    TEMPLATE_LIST_TEST_CASE(
        "object owning observer pointer open chain reversed", "[lifetime][cycles][owner][observer]",
        owner_types) {
        if constexpr (is_cyclic<TestType>) {
            memory_tracker mem_track;
    ...
    TEMPLATE_LIST_TEST_CASE(
        ...
        "[lifetime][cycles][owner][observer]", owner_types) {
        if constexpr (is_cyclic<TestType>) {
            memory_tracker mem_track;
    ...
    TEMPLATE_LIST_TEST_CASE(
...

        {
            ...
            std::generate(vec_own.begin(), vec_own.end(), []() {
                return make_pointer_deleter_1<TestType>();
            });
