...
SUITE {
    ...
    TEMPLATE_LIST_TEST_CASE(
        "observer from this no owner stack", "[observer_from_this]", owner_types) {
                ...
                if constexpr (eoft_always_has_block<TestType>) {
                    ...
                    REQUIRE_THROWS_MATCHES(
                        ...
                        snatch::matchers::with_what_contains{
                            "observer_from_this() called with uninitialized control block"});
                    REQUIRE_THROWS_MATCHES(
                        (make_const_observer_from_this<TestType>(&obj)),
                        oup::bad_observer_from_this,
                        snatch::matchers::with_what_contains{
                            "observer_from_this() called with uninitialized control block"});
                }
    ...
    TEMPLATE_LIST_TEST_CASE(
        "observer from this acquired into base owner as base", "[observer_from_this]",
        owner_types) {
        if constexpr (has_eoft<TestType> && !must_use_make_observable<TestType>) {
            memory_tracker mem_track;
    ...
    TEMPLATE_LIST_TEST_CASE(
        "observer from this acquired into base owner as derived", "[observer_from_this]",
        owner_types) {
        if constexpr (
            has_eoft<TestType> && has_base<TestType> && !must_use_make_observable<TestType>) {
            memory_tracker mem_track;
    ...
    TEMPLATE_LIST_TEST_CASE(
        "observer from this in constructor", "[observer_from_this]", owner_types) {
