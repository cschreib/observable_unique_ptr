...
SUITE {
    TEMPLATE_LIST_TEST_CASE(
        "observer from empty owner constructor", "[construction][observer][from_owner]",
        owner_types) {
    ...
    TEMPLATE_LIST_TEST_CASE(
        "observer from valid owner constructor", "[construction][observer][from_owner]",
        owner_types) {
        memory_tracker mem_track;
    ...
    TEMPLATE_LIST_TEST_CASE(
        "observer from empty owner conversion constructor", "[construction][observer][from_owner]",
        owner_types) {
    ...
    TEMPLATE_LIST_TEST_CASE(
        "observer from valid owner conversion constructor", "[construction][observer][from_owner]",
        owner_types) {
        if constexpr (has_base<TestType>) {
            memory_tracker mem_track;
    ...
    TEMPLATE_LIST_TEST_CASE(
        ...
        "[construction][observer][from_owner]", owner_types) {
        if constexpr (has_base<TestType>) {
            memory_tracker mem_track;

            {
                base_ptr<TestType> ptr_owner;
                {
                    observer_ptr<TestType> ptr{
                        ptr_owner, static_cast<get_object<TestType>*>(nullptr)};
