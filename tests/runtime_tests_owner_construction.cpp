...
SUITE {
    ...
    TEMPLATE_LIST_TEST_CASE(
        "owner acquiring constructor bad alloc with deleter", "[construction][owner]",
        owner_types) {
        if constexpr (!must_use_make_observable<TestType> && has_stateful_deleter<TestType>) {
            memory_tracker mem_track;
    ...
    TEMPLATE_LIST_TEST_CASE(
        "owner acquiring constructor null", "[construction][owner]", owner_types) {
        if constexpr (!must_use_make_observable<TestType>) {
            memory_tracker mem_track;
    ...
    TEMPLATE_LIST_TEST_CASE(
        "owner explicit conversion constructor with custom deleter", "[construction][owner]",
        owner_types) {
    ...
    TEMPLATE_LIST_TEST_CASE(
        ...
        "[construction][owner]", owner_types) {
        if constexpr (has_base<TestType> && has_stateful_deleter<TestType>) {
            memory_tracker mem_track;
