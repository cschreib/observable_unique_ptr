...
constexpr std::size_t max_tests = 5'000;
...
constexpr std::size_t max_expr_length = 1'024;
...
struct registry {
    std::array<test_case, max_tests> test_list;
    std::size_t                      test_count = 0;

    bool verbose = false;

    template<typename T>
    proxy<T> add(std::string_view name, std::string_view tags) {
    ...
    void register_tests(std::string_view name, std::string_view tags, const F& func) {
        (register_test(name, tags, get_type_name<Args>(), to_test_case_ptr<Args>(func)), ...);
    }

    void print_location(const test_case& current_case, const char* filename, int line_number) const;

    void print_failure() const;
    void print_skip() const;
    void print_details(const char* message) const;
    void print_details_expr(const char* check, const char* exp_str, const expression& exp) const;

    void run(test_case& t);
    void set_state(test_case& t, test_state s);
...
#define TESTING_EXPR(x) testing::expression{} <= x

#define TEMPLATE_LIST_TEST_CASE(NAME, TAGS, TYPES)                                                 \
    static const char* TESTING_MACRO_CONCAT(test_id_, __COUNTER__) =                               \
        testing::tests.add<TYPES>(NAME, TAGS) =                                                    \
            []<typename TestType>(testing::test_case & CURRENT_CASE [[maybe_unused]]) -> void
...
#define REQUIRE(EXP)                                                                               \
    do {                                                                                           \
        ...
        if (!(EXP)) {                                                                              \
            ...
            throw testing::test_state::failed;                                                     \
        }                                                                                          \
    } while (0)
...
#define CHECK(EXP)                                                                                 \
    do {                                                                                           \
        ...
        if (!(EXP)) {                                                                              \
            ...
            testing::tests.set_state(CURRENT_CASE, testing::test_state::failed);                   \
        }                                                                                          \
    } while (0)
...
#define SKIP(MESSAGE)                                                                              \
    do {                                                                                           \
        ...
        throw testing::test_state::skipped;                                                        \
    } while (0)
